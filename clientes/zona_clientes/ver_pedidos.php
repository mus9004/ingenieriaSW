...
if(isset($_SESSION["id_cliente"])){
	...
	while($fila1=mysqli_fetch_array($registros1)){
		...
		$registros2=mysqli_query($conexion,"SELECT pedido,producto,cantidad,precio_producto,descargable,id_producto FROM pedidos WHERE pedido='$fila1[pedido]'");
		...
		while($fila2=mysqli_fetch_array($registros2)){
			 ...
			 ?>
        		<tr class="warning">
		 			<td>
					<?php echo $fila2["producto"]; ?>
         			</td>
         			<td>
					<?php echo $fila2["precio_producto"]; ?>
         			</td>
         			<td>
					...
					if($fila2["descargable"]==1){
						?>
                        <img width="20px" src="flechaDescarga.png" >
                        <?php 
					}
					if($fila2["descargable"]==2){
						?>
                        <a href="descargar_zip.php?id_producto=<?php echo $fila2["id_producto"]; ?>&id_cliente=<?php echo $_SESSION["id_cliente"]; ?>&pedido=<?php echo $fila1["pedido"]; ?>">
                        <img width="20px" src="flechaDescarga.png" >
                        </a>
                        <?php 
					}
