...
$registros3=mysqli_query($conexion,"SELECT nombre,apellidos,direccion,cp,provincia FROM clientes WHERE id_cliente='$_POST[id_cliente]'");
$fila3=mysqli_fetch_array($registros3);
echo "Nombre: <strong>".$fila3["nombre"]." ".$fila3["apellidos"]."</strong><br>";
echo "Dirección: C/ <strong>".$fila3["direccion"].", ".$fila3["provincia"]." (".$fila3["cp"].")</strong><br><br>"
?>
<div>
		...
		$fila2=mysqli_fetch_array($registros2);
			...
			<?php
				...
				echo "<div style='float:right'>";
					...
					echo $fila2["total_pedido"]." Euros (Sin IVA)</br>";
					echo "--------------------------------------------</br>";
					echo "+ 21% de IVA</br>";
					echo "--------------------------------------------</br>";
					$total_iva=(($fila2["total_pedido"]*21)/100)+$fila2["total_pedido"];
