...
if(isset($_SESSION["id_cliente"]) && isset($_GET["pedido"])){
	...
	$registros3=mysqli_query($conexion,"SELECT nombre,apellidos,direccion,cp,provincia FROM clientes WHERE id_cliente='$_SESSION[id_cliente]'");
	...
	$pdf->Cell(0,26,"Tienda Online S.L.",0,0,"L");
	
	
	$pdf->SetXY(30,75);
	$pdf->SetFont("Times","",12);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(0,13,"Calle",0,2,"L");
	$pdf->Cell(0,13,"Provincia (C.P.)",0,0,"L");
	...
	$pdf->SetFont("Times","",12);
	$pdf->SetTextColor(0,0,0);
	$pdf->Cell(280,13,utf8_encode($fila3["provincia"]).", C/".utf8_encode($fila3["direccion"])." (".$fila3["cp"].")",0,0,"L");
	$pdf->SetTextColor(0,102,255);
	$pdf->SetFont("Times","B",12);
